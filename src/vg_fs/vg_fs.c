...
vg_fs_register(void) {
    ...
    if (ret != 0) {
        printk(KERN_ERR "vas_log: can't register filesystem\n");
        return false;
    }

    mount_point = kern_mount(&vas_fs_type);
    if (IS_ERR(mount_point)) {
        printk(KERN_ERR "vas_log: kern_mount failed with error %ld\n", PTR_ERR(mount_point));
        unregister_filesystem(&vas_fs_type);
        return false;
    }

    printk(KERN_INFO "vas_log: filesystem registered and mounted successfully\n");
    return true;
}
